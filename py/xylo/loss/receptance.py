...
def loss(cut: t.CutCubic, wood: t.Wood, bar: t.BarProps, ws: jnp.ndarray):
    sections = xylo.cut.cubic(bar, cut)
    wsT = ws.T
    rs  = jax.vmap(lambda w: r.receptance_scalar(wood, bar, sections, w))(wsT[0] * 2 * jnp.pi)
    jax.debug.print("{rs}", rs=rs)
    return -jnp.dot(rs, wsT[1])

def loss_(cut: t.CutCubic, args):
  wood, bar, ws = args
  return loss(cut, wood, bar, ws)
